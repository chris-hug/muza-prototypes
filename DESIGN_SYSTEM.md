...
**A picker sheet's list is capped against what the KEYBOARD leaves, not the viewport.** `max-h-[60vh]` is measured against the full viewport, so with a keyboard up it is taller than the whole sheet: the popup scrolls instead of the list, and the field ends up below the fold. Use `max-h-[min(60vh,calc(100dvh-var(--kb,0px)-Nrem))]`, where `N` is the sheet's other bands (header + gaps + field + any action row).

**Search inside a sheet is a screen, not a field.** Focusing the search input switches the sheet to a **find screen** (`AddMusicDialog`): browse chrome — tabs, the confirming action — steps aside, a back control returns, and the field **floats** over the results (`absolute inset-x-3 bottom-3`, own `bg-popover` + shadow, list padded `pb-20` to scroll under it) instead of holding a band. With no query it shows **recent searches** ([`useRecentSearches`](src/lib/use-recent-searches.ts) — `localStorage`, committed queries only, newest first, capped at 8), or a heading naming the scope when there are none. Render the input **once, outside the branches**, or entering the find screen remounts it and throws away the focus that got you there.
