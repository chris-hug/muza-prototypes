...
`--kb` is published by [`useKeyboardInset`](src/lib/use-keyboard-inset.ts), mounted once in the app shell. It is measured as `documentElement.clientHeight − visualViewport.height − visualViewport.offsetTop` — **`clientHeight`, not `innerHeight`**: a sheet is `position: fixed`, so its `bottom` is the LAYOUT viewport's bottom, which is what `clientHeight` reports, while iOS's `innerHeight` is the large viewport (bottom toolbar collapsed away) and runs a toolbar-height too tall in a browser that keeps one. Open any page with `?kbdebug` to see all of these live on the device (`KeyboardProbe`). Chrome/Android is handled declaratively by `interactive-widget=resizes-content` in the viewport meta, so `--kb` stays 0 there.
