...
contract:
  ...
  - "[sheet] **The viewport meta must not carry `user-scalable=no` or `maximum-scale=1`.** Blocking pinch-zoom is a WCAG 1.4.4 failure, and the double-tap delay it is usually there to prevent is answered by `touch-action: manipulation` instead."
---
