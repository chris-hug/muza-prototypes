...
- `DESIGN_SYSTEM.md:680` and `home.tsx:4209` (the **Search** sections, not the Media List Item ones) still say the `label` kind on a search tab renders "with a 'Label' badge + album count" · the row draws the round avatar only (media-list-item.tsx:172–184) and `search-results-view.tsx` passes the album count as `subtitle` and no badge. The component comment and the DS page's own row description were corrected; these two lines were not.
...
- No source states why the meta breakpoint is 240px rather than another value (media-list-item.tsx:114) — the rationale above stops at "one field to drop"; the number itself is unexplained.
