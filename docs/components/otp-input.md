...
| `InputOTPSlot` | `h-10 w-10 border-y border-r border-border bg-background text-base`; `first:rounded-l-md first:border-l last:rounded-r-md`; active `z-10 ring-3 ring-ring/50` | 40px — the form-control height; the active slot lifts (`z-10`) so its ring is not cut by the neighbour's border |
...
- The slots are `rounded-l-md` / `rounded-r-md` on a `bg-background` box with a plain `border-border`, and focus is `ring-3 ring-ring/50` alone; the form-control recipe (`DESIGN_SYSTEM.md`) is `rounded-full`, `hover:border-foreground/30` and `focus-visible:border-ring` under the ring. A segmented field cannot be a pill, but the hover and the ring's border are missing without a stated reason.
